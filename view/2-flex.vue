<template>
  <div class="wrapper">
    ...
    <div>
      <ul ref="wrapUl" id="wrap-ul" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy" infinite-scroll-distance="10">
        <!-- each component is wrapped by a waterfall slot -->
        <li v-for="(item, index) in studentData" :key="index">
          <div class="wrap">
            <div class="container">
              <p class="stu-img">
    ...
    getFetchStuData (stuName = 'fsstudent', pageNo = this.pageNo) {
      ...
      this.$http.post('api/caseStudentList', {
        ...
        if (response.data.errcode === '0') {
          response.data.result.caseStudentList.result.forEach((v) => {
...
#wrap-ul {
  ...
  flex-direction: row;
  align-items: flex-start;   // 可以让子集高度自适应，否则高度都一样了
  // flex-direction: column;
  // height:500px;  // 高度要一直跟着变化，而且是竖直排列,row的话要设置高度
